...
class Messenger {
        ...
        if (opts.onExtensionRequest) {
            ...
            chrome.runtime.onMessage.addListener(async (data, sender, res) => {
                ...
                try {
                    ...
                    message.error = `Error command ${command} (extension): ${err.message}`;
                }
        ...
        if (opts.onDomRequest) {
            ...
            window.addEventListener("message", async (event) => {
                ...
                try {
                    ...
                    message.error = `Error command ${command} (dom): ${err.message}`;
                }
                if (typeof message.result === "undefined" && !message.error)
                    return;
    ...
    sendResponse(type, message, sender) {
        console.log("sending response ", type, " ", sender === null || sender === void 0 ? void 0 : sender.tab);
        console.log(message);
        if (type === "dom")
...
const signer = {
    ...
    encodeTransaction: async (activeData) => {
        return kondorSigner_messenger.sendDomMessage("signer:encodeTransaction", { activeData });
    },
    decodeTransaction: async (tx) => {
        return kondorSigner_messenger.sendDomMessage("signer:decodeTransaction", { tx });
    },
    sendTransaction: async (tx, abis) => {
        const transaction = await kondorSigner_messenger.sendDomMessage("signer:sendTransaction", {
            tx,
            abis,
        });
        return {
...
async function getAccounts() {
    return kondorAccount_messenger.sendDomMessage("getAccounts", {});
}
